local arb_magus_defeated_trait = {
	["arb_golden_magus"] = {trait = "arb_golden_magus_defeated_trait", special_subtype = nil, trait_prefix = nil}
}


local function golden_magus_get_enemy_legendary_lords_in_last_battle(character)
	...
	for i = 1, num_attackers do
		...
		if char_obj:is_null_interface() == false then
			...
			if arb_magus_defeated_trait[char_subtype] ~= nil then
...
core:add_listener(
	"arb_magus_defeated_trait",
	"CharacterCompletedBattle",
	true,
	function(context)
		local character = context:character()
		if cm:char_is_general_with_army(character) and character:won_battle() then
			local enemy_LL = golden_magus_get_enemy_legendary_lords_in_last_battle(character)
			
			for i = 1, #enemy_LL do
				local LL_details = arb_magus_defeated_trait[enemy_LL[i]]
