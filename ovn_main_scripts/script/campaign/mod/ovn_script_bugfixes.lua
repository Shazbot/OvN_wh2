...
--- Halfling faction has the old Empire tech tree above it.
--- So we hide techs that are below on the y axis of our first tech.
local function hide_halfling_tech_overflow()
	local tree_parent = find_ui_component_str("root > technology_panel > listview > list_clip > list_box > emp_civ_reworkd > tree_parent")

	local a = find_ui_component_str(tree_parent, "slot_parent > elo_halfling_training")
...
core:remove_listener("ovn_hide_halfling_tech_overflow")
core:add_listener(
	"ovn_hide_halfling_tech_overflow",
	"PanelOpenedCampaign",
	function(context)
		return context.string == "technology_panel" and cm:get_local_faction_name() == "wh2_main_emp_the_moot"
	end,
	function()
		hide_halfling_tech_overflow()
	end,
	true
)
