...
local hlf_olorin_defeated_trait = {
...
local function olorin_get_enemy_legendary_lords_in_last_battle(character)
	...
	for i = 1, num_attackers do
		...
		if char_obj:is_null_interface() == false then
			...
			if hlf_olorin_defeated_trait[char_subtype] ~= nil then
...
core:add_listener(
	...
	function(context)
		...
		if cm:char_is_general_with_army(character) and character:won_battle() then
			...
			for i = 1, #enemy_LL do
				local LL_details = hlf_olorin_defeated_trait[enemy_LL[i]]
